...
bool dfsCheck(TreeWithCentroidDecomposition& tree, vector<char>& rank, int u, int p, int level) {
    if (level > 26)
        return false;

    rank[u] = 'A' + level;
    for (auto v : tree.ctEdges[u]) {
        if (v == p)
            continue;
        if (!dfsCheck(tree, rank, v, u, level + 1))
            return false;
    }
    return true;
}
...
int main(void) {
    ...
    vector<char> rank(N);
    bool ans = dfsCheck(tree, rank, tree.ctRoot, -1, 0);
