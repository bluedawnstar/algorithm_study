...
int main(void) {
    ...
    vector<int> parent(N);
    auto tree = makeLinkCutTreePathQueryArray(N, [](int a, int b) { return a + b; }, [](int x, int n) { return x * n; }, 0);
    for (int i = 1; i < N; i++) {
        int u;
        cin >> u;
        u--;
        ...
        tree.update(i, color[i]);
        parent[i] = u;
    }
    tree.update(0, color[0]);
    parent[0] = -1;
    ...
    while (Q-- > 0) {
        ...
        if (t == 'T') {
            ...
            int nu = tree.queryToAncestor(u, lca);
            int nv = tree.queryToAncestor(v, lca);
            int n = tree.query(u, v);
            if (n < k)
                cout << -1 << endl;
            else if (k <= nu) {
                u = tree.climbKth(u, k, [](int x) { return x; });
                cout << u + 1 << endl;
            } else {
                k = n - k + 1;
                v = tree.climbKth(v, k, [](int x) { return x; });
                cout << v + 1 << endl;
            }
        }
    }
