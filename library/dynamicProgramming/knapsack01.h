...
struct Knapsack01 {
    ...
    static int knapsackNaive(const vector<int>& weight, const vector<int>& value, int maxWeight) {
    ...
    // maxValue <<< maxWeight
    // O(n^2*maxValue)
    static int knapsack(const vector<int>& weight, const vector<int>& value, int maxWeight, int maxValue) {
        if (maxWeight <= 0)
            return 0;
