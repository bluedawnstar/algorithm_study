...
// Upper envelope (simple version of PersistentLowerConvexHull)
template <typename T>
struct DynamicLowerConvexHull {
    vector<pair<T, T>>  points;

    DynamicLowerConvexHull(int N) {
        points.reserve(N + 1);
    }
    ...
    void addLinear(T x, T y) {
        ...
        auto base = make_pair(x, y);
        while (points.size() > 1 && ccw(base, points[points.size() - 1], points[points.size() - 2]))
            points.pop_back();
