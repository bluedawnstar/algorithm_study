...
struct FactorialMod {
    // Returns value of n! % P, O(n)
    static int factorialNaive(int n, int P) {
        if (n >= P)
            return 0;
    ...
    */
    ...
    static int factorialWilson(int n, int M) {
        // n! % M is 0 if n >= M
        if (n >= M)
            return 0;
    ...
    static long long factorialWilson(long long n, long long M) {
        // n! % M is 0 if n >= M
        if (n >= M)
            return 0;
