...
#include "segmentTreePersistentLazyRollbackable.h"
...
void testRollbackableSegmentTreePersistentLazy() {
    //return; //TODO: if you want to test, make this line a comment.

    cout << "--- Rollbackable Lazy Persistent Segment Tree -----------------------------" << endl;
    {
        const int N = 1000;
        vector<int> in(N);
        ...
        RollbackablePersistentSegmentTreeLazy<int> tree(in,
        ...
        for (int i = 0; i < N; i++) {
            ...
            int ans;
            tie(ans, root) = tree.lowerBound(root, [sum](int val) { return val >= sum; });
    ...
    cout << "OK!" << endl;
    {
        ...
        for (int j = 0; j < T; j++) {
            for (int i = 0; i < N; i++) {
                ...
                int ans;
                tie(ans, roots[j]) = tree.lowerBound(roots[j], [sum](int val) { return val >= sum; });
    ...
    cout << "OK!" << endl;
    {
        ...
        RollbackablePersistentSegmentTreeLazy<int> tree(N, [](int a, int b) { return a + b; }, [](int a, int n) { return a * n; }, 0);
        ...
        for (int i = 0; i < T; i++) {
            ...
            int x = RandInt32::get() % 1000;
            {
                ...
                int ans;
                tie(ans, root) = tree.query(root, L, R);
                assert(gt == ans);
            }
            {
                int t;
                tie(t, root) = tree.query(root, 0, L);
                int sum = t + RandInt32::get() % 10;
                int gt = lowerBoundSlow(vv.back(), sum);
                int ans;
                tie(ans, root) = tree.lowerBound(root, [sum](int x) { return sum <= x; });
                assert(gt == ans);
            }
    ...
    }
    {
        ...
        for (int t = 0; t < T; t++) {
            ...
            RollbackablePersistentSegmentTreeLazy<long long> tree(
            ...
            for (int i = 0; i < T; i++) {
                ...
                long long ans;
                tie(ans, roots[history]) = tree.query(roots[history], L, R);
