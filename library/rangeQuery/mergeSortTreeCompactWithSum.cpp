...
#include "mergeSortTreeCompactWithSum.h"
...
static void test(vector<int>& in, CompactMergeSortTreeWithSum<int,int>& tree, int N, int L, int R, int K) {
...
void testMergeSortTreeCompactWithSum() {
    ...
    for (int i = 0; i < T; i++) {
        ...
        CompactMergeSortTreeWithSum<int,int> tree(in);
        for (int j = 0; j < T; j++) {
            int K = RandInt32::get() % N;
