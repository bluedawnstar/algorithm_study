...
struct PersistentSegmentTreeLazyWithBase {
    enum LazyT {
        lzNone,
        lzSet,
        lzUpdate
    ...
    PersistentSegmentTreeLazyWithBase(MergeOp mop, T dflt = T())
        : N(0), nodes(), initRoot(-1), defaultValue(dflt), mergeOp(mop) {
    }

    PersistentSegmentTreeLazyWithBase(int n, MergeOp mop, T dflt = T())
        : defaultValue(dflt), mergeOp(mop) {
        build(defaultValue, n);
    }

    PersistentSegmentTreeLazyWithBase(T value, int n, MergeOp mop, T dflt = T())
        : defaultValue(dflt), mergeOp(mop) {
        build(value, n);
    }

    PersistentSegmentTreeLazyWithBase(const T A[], int n, MergeOp mop, T dflt = T())
        : defaultValue(dflt), mergeOp(mop) {
        build(A, n);
    }

    PersistentSegmentTreeLazyWithBase(const vector<T>& A, MergeOp mop, T dflt = T())
        : defaultValue(dflt), mergeOp(mop) {
        build(A);
    }
...
inline PersistentSegmentTreeLazyWithBase<T, MergeOp>
makePersistentSegmentTreeLazyWithBase(MergeOp mop, T dfltValue = T()) {
    return PersistentSegmentTreeLazyWithBase<T, MergeOp, BlockOp>(mop, dfltValue);
}

template <typename T, typename MergeOp>
inline PersistentSegmentTreeLazyWithBase<T, MergeOp>
makePersistentSegmentTreeLazyWithBase(int n, MergeOp mop, T dfltValue = T()) {
    return PersistentSegmentTreeLazyWithBase<T, MergeOp>(n, mop, dfltValue);
}

template <typename T, typename MergeOp>
inline PersistentSegmentTreeLazyWithBase<T, MergeOp>
makePersistentSegmentTreeLazyWithBase(T value, int n, MergeOp mop, T dfltValue = T()) {
    return PersistentSegmentTreeLazyWithBase<T, MergeOp>(value, n, mop, dfltValue);
}

template <typename T, typename MergeOp>
inline PersistentSegmentTreeLazyWithBase<T, MergeOp>
makePersistentSegmentTreeLazyWithBase(const T A[], int n, MergeOp mop, T dfltValue = T()) {
    return PersistentSegmentTreeLazyWithBase<T, MergeOp>(A, n, mop, dfltValue);
}

template <typename T, typename MergeOp>
inline PersistentSegmentTreeLazyWithBase<T, MergeOp>
makePersistentSegmentTreeLazyWithBase(const vector<T>& A, MergeOp mop, T dfltValue = T()) {
    return PersistentSegmentTreeLazyWithBase<T, MergeOp>(A, mop, dfltValue);
}
