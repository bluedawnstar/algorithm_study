...
#include "segmentTreePersistentLazyWithBase.h"
...
static vector<long long> fastSolver(const vector<int>& vertexWeight, const vector<int>& edgeWeight,
    ...
    PersistentSegmentTreeLazyWithBase<long long> solver(N, [](long long a, long long b) {
        return min(a, b);
    ...
    for (int R = 1; R < N; R++) {
        ...
        for (auto& it : qry[R]) {
            int L = it.first;
            int idx = it.second;
            res[idx] = solver.query(root, L, R - 1);
        }
...
void testPersistentSegmentTreeLazyWithBase() {
    //return; //TODO: if you want to test, make this line a comment.

    cout << "--- Persistent Segment Tree with Lazy Propagation and Base Value ---------------------------" << endl;
    {
