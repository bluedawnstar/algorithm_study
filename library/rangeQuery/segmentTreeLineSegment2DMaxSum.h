...
struct SegmentTreeLineSegment2DMaxSum {
    ...
    //    for (j = i; j <= right; j++)
    //       sum += max[k=i..j]{ Fk(i) }, Fk(x) = b[k] + m[k] * x
    vector<long long> sum;

    // for (i = left; i <= right; i++)
    //    for (j = i; j <= right; j++)
    //       sumX += max[k=i..j]{ Fk(i) * i },  Fk(x) = b[k] + m[k] * x
    vector<long long> sumX;

    vector<long long> sumXTbl;  // SUM[0<=x<=n]{ x }
    vector<long long> sumXXTbl; // SUM[0<=x<=n]{ x * x }

    SegmentTreeLineSegment2DMaxSum() : N(0) {
    }

    explicit SegmentTreeLineSegment2DMaxSum(int size) {
        init(size);
    }
