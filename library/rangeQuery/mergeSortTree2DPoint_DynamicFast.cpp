...
#include "mergeSortTree2DPoint_DynamicFast.h"
...
void testMergeSortTree2DPoint_DynamicFast() {
    return; //TODO: if you want to test, make this line a comment.

    cout << "--- Fast Dynamic Merge Sort Tree of 2D Point ---------------------------------------" << endl;
    {
        int H = 1000000;
        int N = 10000;
        int T = 10000;
        ...
        FastDynamicMergeSortTree2DPoint tree(N);
    ...
    cout << "OK!" << endl;
    {
        ...
        FastDynamicMergeSortTree2DPoint tree2(N);

        PROFILE_START(0);
        for (int i = 0; i < T; i++)
            tree.add(points[i].first, points[i].second);
        PROFILE_STOP(0);

        PROFILE_START(1);
        for (int i = 0; i < T; i++)
            tree2.add(points[i].first, points[i].second);
        PROFILE_STOP(1);
        ...
        for (int i = 0; i < T; i++) {
            int x1, y1, x2, y2;
            tie(x1, y1, x2, y2) = rects[i];
            ans += tree.count(x1, y1, x2, y2);
        }
        cout << ans << endl;
        PROFILE_STOP(2);
