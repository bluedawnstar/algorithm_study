...
struct CompactMergeSortTreeWithLiChaoTreeMax {
    ...
    CompactMergeSortTreeWithLiChaoTreeMax() : minX(-INF), maxX(INF), N(0) {
    }

    CompactMergeSortTreeWithLiChaoTreeMax(int minX, int maxX) : minX(minX), maxX(maxX), N(0) {
    }

    CompactMergeSortTreeWithLiChaoTreeMax(int minX, int maxX, const pair<long long, long long> lines[], int n)
        : minX(minX), maxX(maxX), N(0) {
        build(lines, n);
    }

    explicit CompactMergeSortTreeWithLiChaoTreeMax(int minX, int maxX, const vector<pair<long long, long long>>& lines)
        : minX(minX), maxX(maxX), N(0) {
        build(lines);
    }

    ~CompactMergeSortTreeWithLiChaoTreeMax() {
        for (auto* p : nodes)
            delete p;
    }
    ...
    long long query(int left, int right, int X) const {
        ...
        for (int L = left + N, R = right + N; L <= R; L >>= 1, R >>= 1) {
            if (L & 1)
                res = max(res, query(L++, X));
            if ((R & 1) == 0)
                res = max(res, query(R--, X));
        }
