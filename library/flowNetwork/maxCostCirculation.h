...
struct MaxCostCirculation {
    ...
    MaxCostCirculation() : N(0) {
    }

    explicit MaxCostCirculation(int n) {
        init(n);
    }
    ...
    // return (total cost of positive cycles), O(flow * C * VE^2)
    // -> side-effect: capacity values of edges are updated (positive-cycles cancelled)
    T solve() {
        T ans = 0;

        int root = -1;
        while ((root = positiveCycle()) != -1) {
            vector<bool> mark(N);
