...
vector<T> convolution(const vector<T>& x, const vector<T>& h) {
    int xN = (int)x.size();
    int hN = (int)h.size();
...
// y(n) = SUM x(t) * h(n - t)
vector<int> convolutionMod(const vector<int>& x, const vector<int>& h, int MOD) {
    int xN = (int)x.size();
    int hN = (int)h.size();
...
void testConvolution() {
    ...
    cout << x << " * " << h << " : ";
    cout << v2 << endl;
    assert(operator ==(v2, vector<int>{ 1, 4, 7, 6 }));

    v2 = convolutionFFT(x, h, false);
    cout << x << " * " << h << " : ";
    cout << v2 << endl;
    assert(operator ==(v2, vector<int>{ 2, 5, 8, 3 }));
