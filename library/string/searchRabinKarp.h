...
inline vector<int> searchRabinKarpLowercaseAlphabet(const string& text, const string& pattern) {
    const unsigned long long d = 31ll;
    ...
    for (int i = 0; i < M; i++) {
        pattHash = pattHash * d + (pattern[i] - 'a' + 1);   // hash of pattern
        textHash = textHash * d + (text[i] - 'a' + 1);      // hash of text
    }
    ...
    for (int i = 0; i <= N - M; i++) {
        ...
        // next hash value
        if (i < N - M) {
            textHash = d * textHash - (text[i] - 'a' + 1) * dM + (text[i + M] - 'a' + 1);
        }
    }
