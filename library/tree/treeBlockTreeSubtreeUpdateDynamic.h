...
struct DynamicBlockTreeSubtreeUpdate {
    ...
    void changeParent(int u, int newParent) {
        ...
        //.. pathValuesInBlock
        //.. subtreeValuesInBlock
#if 0
        unlink(u);

        if (blockRoot[oldParent] != newParent[newParent]) {
            vector<int> subBlocks;
            dfsGetSubBlocks(u, parent[u], blockRoot[u], subBlocks);
        }
...
protected:
    ...
    void unlink(int v) {
        ...
        if (blockRoot[v] != v) {
            int blockId = blockRoot[v];
            int sizeDelta = treeSizeInBlock[v];
            while (u >= 0 && blockRoot[u] == blockId) {
                auto t = values[u];
                for (auto v : edges[u]) {
                    if (v != parent[u] && blockRoot[v] != v)
