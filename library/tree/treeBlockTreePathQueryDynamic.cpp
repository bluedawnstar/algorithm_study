...
#include "treeBlockTreePathQueryDynamic.h"
...
static int pathQueryNaive(DynamicBlockTreePathQuery<int,MergeOp>& tree, int u, int v, vector<int>& values) {
    if (u == v)
        return values[u];
...
static int climbKthNaive(DynamicBlockTreePathQuery<int, MergeOp>& tree, int u, int kth, vector<int>& values) {
    while (u >= 0 && kth > 0) {
        if (values[u]) {
            if (--kth == 0)
                break;
...
void testDynamicBlockTreePathQuery() {
    return; //TODO: if you want to test a split function, make this line to a comment.

    cout << "--- Dynamic Block Tree for Path Query ---------------------" << endl;
    // basic test
    {
        int N = 10000;
        int T = 10000;
        ...
        Tree tree(N);
        auto treeSqrt = makeDynamicBlockTreePathQuery(N, MergeOp(), 0);
