...
#include "treeSqrtDecompositionPathQueryDynamic.h"
...
static int pathQueryNaive(DynamicTreeSqrtDecompositionPathQuery<int,MergeOp>& tree, int u, int v, vector<int>& values) {
    if (u == v)
        return values[u];
...
static int climbKthNaive(DynamicTreeSqrtDecompositionPathQuery<int, MergeOp>& tree, int u, int kth, vector<int>& values) {
    while (u >= 0 && kth > 0) {
        if (values[u]) {
            if (--kth == 0)
                break;
...
void testDynamicTreeSqrtDecompositionPathQuery() {
    ...
    // basic test
    {
        ...
        Tree tree(N);
        auto treeSqrt = makeDynamicTreeSqrtDecompositionPathQuery(N, MergeOp(), 0);
